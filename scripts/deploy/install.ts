...
async function install() {
  ...
  completeHealth();
  io.write(
    `\nInstallation complete. Open ${new URL("/setup", answers.siteUrl).toString()}`,
  );
  io.write(
    "Next: authenticate with the provider configured during deployment and complete the one-time first-launch setup.",
  );
  io.write(
