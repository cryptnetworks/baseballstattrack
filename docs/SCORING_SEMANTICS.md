...
| `RunnerPlayRecorded`        | Runner play          | Records every affected runner in one standalone live play.                                     | Play type, complete movement ledger, fielding credits, responsible catcher where applicable.                  | Bases, outs, score, inning.                                 | All movements validate against one pre-play state and accept atomically.                       | Runs, earned runs, SB, CS, putouts, assists, errors.                              | Implemented in event schema v3.                      |
...
The implemented issue #11 formulas, exact numeric representation, event-schema v2 earned-run judgment, event-schema v3 runner-play extension, correction behavior, supported season boundary, and explicit unsupported cases are documented in [STATISTIC_DERIVATION.md](STATISTIC_DERIVATION.md). This implementation note narrows representability gaps without changing the source-authority rules below.
