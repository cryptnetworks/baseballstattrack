...
`corrected` is a review state, not permission to continue live scoring. After correction review, the game transitions back to `verified` or remains `completed` if review is not accepted.
...
- Effective game revision or parent event relationship.
...
- Purpose and classification: each event is one of lifecycle, setup, plate appearance, runner movement, out, fielding judgment, pitching/battery award, substitution, correction, or verification.
...
- Runner effects: every runner movement, score, award, or out must be recorded as `RunnerAdvanceRecorded`, `RunnerOutRecorded`, or a specialized event that expands to the same runner ledger.
...
- Correction behavior: an event is never mutated or deleted; corrections append `CorrectionApplied` events that supersede, reverse, or replace one event, a dependent event range, or a scorer judgment.
...
| `RunnerAdvanceRecorded`       | Runner movement        | Records runner movement caused by the responsible event.                                       | Runner, start base, end base or out, responsibility, forced flag, earned/unearned judgment where relevant.    | Bases, score, outs.                                         | Start base must be occupied unless runner is batter-runner; end base must be valid and unique. | Runs, RBI eligibility, earned runs, stolen bases, caught stealing, outs on bases. | Recorded as part of play or standalone runner event. |
...
| `PitchOrBatteryAwardRecorded` | Pitching/battery award | Records wild pitch, passed ball, balk, ground-rule double, automatic double, or similar award. | Type, responsible pitcher/catcher or rule, affected runners, advances.                                        | Bases, score.                                               | Runner advances must be complete and legal for the award.                                      | WP, PB, balk, base awards, earned-run context.                                    | Recorded.                                            |
...
Ground-rule and automatic doubles use the `double` batter credit when the batter is credited with a hit. The award itself is recorded on the runner ledger, usually through `PitchOrBatteryAwardRecorded` or linked `RunnerAdvanceRecorded` entries, so replay knows which runners are entitled to score or advance. If a ruleset awards two bases without hit credit, the event records the award without changing the batter credit to `double`.
...
- Ending base: `first`, `second`, `third`, `home`, or `out`.
...
- Earned/unearned judgment where a scoring rule requires scorer interpretation.
...
- RBI eligibility is derived when mechanical, but explicit scorer judgment is recorded for ambiguous fielder's choice, error, sacrifice, and double-play cases.
...
| Wild pitch, passed ball, and balk            | `PitchOrBatteryAwardRecorded` records award type, responsible player/rule, affected runners, and earned-run context.                               |
