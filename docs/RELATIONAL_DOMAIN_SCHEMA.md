...
`20260801070000_discord_update_delivery` adds deduplicated version-pinned
evaluations, ordered leased Discord deliveries, immutable attempt evidence,
retry/dead-letter state, retention indexes, and service-only RLS.
`20260801071000_discord_update_delivery_indexes` adds the composite
foreign-key access paths identified by the Supabase performance advisor.

`20260803163000_provider_neutral_authentication` adds application-owned
provider identities, rotating and revocable sessions, append-only session
evidence, one-time OAuth attempts, and a non-destructive legacy identity
backfill. Applied production migrations are never edited.
