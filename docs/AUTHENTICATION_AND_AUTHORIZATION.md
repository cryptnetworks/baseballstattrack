...
## Review-derived authorization regression cases

Future authorization tests must exercise these adversarial cases in addition to ordinary allow paths:

| Case                                                                                                                                               | Required result                                                                                                                   |
| -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Administrator attempts to promote an owner, remove the final active owner, or grant themselves owner-equivalent authority.                         | Deny before mutation; audit the high-risk denial.                                                                                 |
| Coach attempts to grant themselves `game.verify`; scorekeeper attempts `game.reopen`.                                                              | Deny because membership grant management cannot self-escalate and neither role has the capability by default.                     |
| Invitation is forwarded, replayed, revoked while accepted, superseded after role/scope change, or accepted by a different provider identity.       | Deny unless the single atomic pending-state transition succeeds for the intended identity and immutable issued snapshot.          |
| Team-scoped coach requests another team in the same season; game-scoped viewer requests season aggregate.                                          | Deny because scope inheritance is exact and neither relationship is inherited.                                                    |
| Viewer requests a report, projection, roster, or export that could disclose private player fields.                                                 | Return only the report type's minimum-field allowlist; require `player.private_view` and export capability for any private field. |
| Idempotent scoring retry, batch, export, projection worker, or report reader runs after access removal or without account context.                 | Reauthorize every protected target or isolated account invocation; deny and do not return protected payload/artifact on failure.  |
| Required audit writer is unavailable for ownership, membership, invitation, verification, reopen, correction, export, recovery, or privacy action. | Fail closed or use a durable transactional outbox; never commit the privileged action silently.                                   |
