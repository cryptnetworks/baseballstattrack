...
- Production deployment must use the immutable migration image and Prisma's
  production deployment flow. Migration credentials should be separate from
  least-privilege runtime credentials when supported.
...
Production bootstrap data is limited to minimal system-owned records, such as
accepted default ruleset versions. Demo, test, preview, and synthetic fixture
data must never be loaded into a production Account or production database.
Production bootstrap is append-safe, auditable, and must not destructively
reseed an existing installation.
...
- Non-production and production databases must remain separate.
...
- Non-production environments must not clone or expose production personal data
  unless a future privacy/security policy explicitly approves anonymized copies.
- Runtime logs and error messages must not print database URLs, direct URLs,
  service-role secrets, or connection strings.
- Production database access must be auditable and limited to approved operators or automation.
- Real values belong only in protected deployment secrets.
